...
for N in N_list:
    for on_over_n in on_over_n_list:
        ...
        for JS_threshold, MD_threshold in zip(JS_thresholds, MD_thresholds):
            time, nmi, mod_ndocd, mod_base, n_coms, n_coms_base = test_ndocd(folder, file_appending="b1_" + str(N) + str(on_over_n), MD_threshold=MD_threshold, JS_threshold=JS_threshold, weighted=True)
        ...
        labels.append(f"N = {N}, on = {int(N*on_over_n)}")
        modularities_base.append(mod_base)
        n_communities_base.append(n_coms_base)
        print(f'\n\n Ended: N - {N} \n\n')


plot_measure_results_data(times, x=JS_thresholds, labels=labels, title_base="Time for different number of vertices", title_ending="", ylabel="Time", xlabel="JS and MD thresholds", save_name="plots/weighted/b1z_time")
plot_measure_results_data(nmis, x=JS_thresholds, labels=labels, title_base="Mutual information for different number of vertices", title_ending="", ylabel="Normalized mutual information", xlabel="JS and MD thresholds", save_name="plots/weighted/b1z_nmi")
plot_measure_results_data(modularities, error=modularities_base, x=JS_thresholds, labels=labels, title_base="Modularities for different number of vertices", title_ending="", ylabel="Link belong modularity", xlabel="JS and MD thresholds", save_name="plots/weighted/b1z_modularity")
plot_measure_results_data(n_communities, error=n_communities_base, x=JS_thresholds, labels=labels, title_base="Communities for different number of vertices", title_ending="", ylabel="Number of communities", xlabel="JS and MD thresholds", save_name="plots/weighted/b1z_ncoms")
pickle.dump((times, nmis, modularities, n_communities, labels, modularities_base, n_communities_base, JS_thresholds), open("data/benchmark_weighted/b1z", 'wb'))
