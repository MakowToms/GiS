...
def plot_measure_results_data(errors, error=None, error_type=None, title_base='MSE', title_ending=' through epochs', labels=["base model", "momentum model", "RMSProp model"], colors=["green", "blue", "red", "magenta", "black", "brown", "orange", "yellow",  "red", "magenta"], xlabel='epochs', ylabel='MSE', from_error=0, show=True, x=None, save_name=None, log=False, log_y=False):
    n = len(errors[0])
    if x is None:
    ...
    if error is not None:
        if error_type is not None:
            for i in range(len(errors)):
                plt.plot(x, error[i][from_error:], color=colors[i], linestyle=':')
        else:
            for i in range(len(errors)):
...
def create_3_plots(nmis, modularities, n_communities, modularities_base, n_communities_base, labels, x, xlabel, base_save_name):
    plot_measure_results_data(nmis, x=x, labels=labels, title_base="Mutual information", title_ending="", ylabel="Normalized mutual information", xlabel=xlabel, save_name="plots/" + base_save_name + "1_nmi")
    plot_measure_results_data(modularities, error=modularities_base, error_type="", x=x, labels=labels, title_base="Modularities", title_ending="", ylabel="Link belong modularity", xlabel=xlabel, save_name="plots/" + base_save_name + "1_modularity")
    plot_measure_results_data(n_communities, error=n_communities_base, error_type="", x=x, labels=labels, title_base="Communities", title_ending="", ylabel="Number of communities", xlabel=xlabel, save_name="plots/" + base_save_name + "1_ncoms")
